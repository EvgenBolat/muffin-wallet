...
7. Дождаться полного развёртывания подов. Можно наблюдать через команду
   ...
   ```
8. В local_env выполнить команды:
   ```
   python3 -m venv venv
...
9. Установить пакет для работы с запросами:
   ```
   pip install requests
   ```
10. Проверить, какой выбран компилятор запуска python (нужно выбрать pyvenv.cfg в папке venv)

11. Запустить скрипт командой:
    ```
    python ./load.py
    ```
...
3. 99-й персентиль времени ответа HTTP (обработка запросов).
   ...
   График корректен, потому что:
   - используется метрика http_server_requests_seconds_max, отражающая реальное максимальное время обработки запросов
   - применяется max_over_time, что позволяет учитывать пиковые значения задержек в заданном временном окне
   - значения логично коррелируют с интенсивностью нагрузки и типом выполняемых операций

   Данный график демонстрирует максимальное время обработки HTTP-запросов по основным эндпоинтам сервиса. В начале нагрузочного теста наблюдается рост задержек, связанный с одновременным выполнением большого количества запросов. По мере завершения потоков нагрузка снижается, что приводит к уменьшению времени обработки. Наибольшие значения зафиксированы для операций транзакций, что объясняется их повышенной сложностью и использованием базы данных. Полученные результаты подтверждают ожидаемое поведение системы под нагрузкой.
